...
class _TT {
  ...
  _TT(int sizePow2)
    : _tab = List<_TTEntry?>.filled(
        sizePow2,
        null,
      ); // تهيئة الجدول بالحجم المطلوب

  _TTEntry? probe(int key) {
    // probe: محاولة قراءة إدخال من الجدول
    final idx = key & (_tab.length - 1); // استخدام bitmask بدل % لتحسين الأداء
    final e = _tab[idx]; // جلب الإدخال في هذا الموضع
    if (e != null && e.key == key)
      return e; // التحقق من تطابق المفتاح (تجنب الاصطدام)
...
class _Killers {
  ...
  _Killers(int maxDepth)
    : k = List.generate(
        maxDepth + 4,
        (_) => [null, null],
      ); // مساحة إضافية احتياطية

  void add(int d, Move m) {
    // إضافة حركة قاتلة لعمق d
    if (m.isCapture) return; // لا نضيف الالتقاطات (هي قوية بذاتها بترتيب آخر)
    if (k[d][0] != m) {
      // إن لم تكن الحركة هي الأولى
  ...
  bool isKiller(int d, Move m) =>
      !m.isCapture &&
      (k[d][0] == m || k[d][1] == m); // التحقق إن كانت الحركة قاتلة
}
...
class _History {
  ...
  int score(PieceColor side, Move m) =>
      h[_sideIdx(side)][_sq(m.start)][_sq(m.end)]; // إرجاع النقاط لحركة ما

  void reward(PieceColor side, Move m, int depth) {
    // مكافأة الحركة الناجحة (مربوطة بالعمق^2)
    h[_sideIdx(side)][_sq(m.start)][_sq(m.end)] += depth * depth;
  }

  static int _sq(Cell c) => c.row * 8 + c.col; // تحويل خلية إلى فهرس [0..63]
  static int _sideIdx(PieceColor s) =>
      (s == PieceColor.white) ? 0 : 1; // الأبيض=0، الأسود=1
}
...
class Deadline {
  ...
  Deadline(int timeMs)
    : _deadlineMs = timeMs,
      _startMs = DateTime.now().millisecondsSinceEpoch; // حفظ البداية
  bool get expired =>
      DateTime.now().millisecondsSinceEpoch - _startMs >=
      _deadlineMs; // تحقق من الانقضاء
}
...
class AiEngine {
  ...
  Future<Move?> search(Board root) async {
    ...
    for (int depth = 1; depth <= cfg.maxDepth; depth++) {
      // Iterative Deepening: من العمق 1 للأقصى
      tt.age++; // زيادة عمر الجدول (يُستخدم في سياسة الاستبدال)
      int alpha =
          lastScore -
          cfg.aspirationDelta; // نافذة Aspiration: البداية حول lastScore
      int beta = lastScore + cfg.aspirationDelta; // الحد العلوي للنافذة
      int tries = 0; // محاولات توسيع النافذة عند الفشل

      while (true) {
        // حلقة تكرار لتوسيع النافذة إذا فشلنا
        final ctx = _Ctx(
          this,
          timer,
          depth,
        ); // سياق البحث: يحتوي killers/history/timer
        ...
        if (score <= alpha && tries < 2) {
          // فشل منخفض (Fail-low): وسّع النافذة للأسفل
          alpha -= cfg.aspirationDelta * 2;
          tries++;
          continue;
        }
        if (score >= beta && tries < 2) {
          // فشل مرتفع (Fail-high): وسّع النافذة للأعلى
  ...
  int _alphaBeta(
    ...
    if (tte != null && tte.depth >= depth) {
      // إذا كان الإدخال أعمق أو مساويًا لعمقنا
      if (tte.flag == EXACT) return tte.score; // قيمة دقيقة: نعيدها مباشرة
      if (tte.flag == LOWER && tte.score > alpha)
        alpha = tte.score; // حد سفلي: ارفع alpha
      if (tte.flag == UPPER && tte.score < beta)
        beta = tte.score; // حد علوي: اخفض beta
    ...
    if (cfg.enableNullMove &&
      ...
      if (nullScore >= beta)
        return nullScore; // إذا تجاوز الحد → قطع (fail-high)
    }

    // --- 4) توليد النقلات القانونية ---
    var moves =
        b.getAllLegalMovesForCurrentPlayer(); // جميع الحركات القانونية للوضعية الحالية
    ...
    for (int i = 0; i < moves.length; i++) {
      ...
      final isQuiet =
          !m.isCapture && !_givesCheck(b, m); // حركة هادئة: لا التقاط ولا كش
      ...
      b.makeMove(m); // إنشاء لوحة جديدة بتنفيذ الحركة (Immutable API)

      int score; // نتيجة الفرع
      // --- 7) PVS: بحث ضيق لجميع النقلات بعد الأولى ---
      if (!cfg.enablePVS || i == 0) {
        // النقلات الأولى (PV) نبحث بنافذة كاملة
        score = -_alphaBeta(b, newDepth, -beta, -alpha, ctx);
      } else {
        // النقلات اللاحقة نبحث بنافذة ضيقة (α, α+1)
      ...
      if (score > bestScore) {
        ...
        if (bestScore > alpha) {
          // توسيع alpha إذا تحسننا
          alpha = bestScore;
          if (alpha >= beta) {
            // قطع (beta cutoff)
            if (isQuiet) {
              // مكافأة Killer/History للحركات الهادئة التي قطعت
  ...
  int _quiescence(Board b, int alpha, int beta, _Ctx ctx) {
    ...
    for (final m in all) {
      if (m.isCapture || _givesCheck(b, m))
        qMoves.add(m); // احتفظ فقط بالالتقاطات/الكش
    }
    ...
    for (final m in qMoves) {
      // جرّب كل نقلة هادرة
      b.makeMove(m); // طبق الحركة على لوحة جديدة
      final score = -_quiescence(b, -beta, -alpha, ctx);
      b.unMakeMove(move: m);
      // بحث递归 مع نوافذ معكوسة
  ...
  bool _givesCheck(Board b, Move m) {
    ...
    final opp =
        b.currentPlayer == PieceColor.white
            ? PieceColor.black
            : PieceColor.white; // لون الخصم (قبل التنفيذ)
    b.makeMove(m); // نفّذ الحركة على نفس اللوحة
    final res = b.isKingInCheck(opp); // تحقق: هل ملك الخصم في كش؟
    b.unMakeMove(move: m); // ارجع للحالة السابقة بسرعة
    return res;
  }

  bool _hasNonPawnMaterial(Board b) {
    // هل يوجد قطع غير البيادق؟ (لتفعيل null move)
    // لتبسيط المثال نرجع true دائمًا. يمكن لاحقًا إضافة فحص فعلي عبر Board.
    return true;
  }

  Board _makeNull(Board b) {
    // تنفيذ "حركة فارغة" (null move)
  ...
  int _lmrReduction(int depth, int moveIdx) {
    // حساب مقدار تقليل العمق (LMR)
    final r =
        (math.log(depth + 1) * math.log(moveIdx + 1))
            .floor(); // صيغة لوغاريتمية شائعة
    return r.clamp(1, depth - 1); // لا تقلل أكثر من (depth-1) ولا أقل من 1
  }

  int _computeExtension(Board b, Move m) {
    // حساب الامتدادات (Extensions) البسيطة
